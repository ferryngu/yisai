...
- (void)changeMovToMp4:(NSURL *)mediaURL dataBlock:(void (^)(UIImage *movieImage))handler {
    ...
    self.videoPath = [basePath stringByAppendingPathComponent:videoName];
...
- (NSString *)getVideoCachePath {
    ...
    NSString *videoCache = [docPath stringByAppendingPathComponent:@"Movie"] ;
...
- (NSString *)getUploadFile_type:(NSString *)type fileType:(NSString *)fileType {
    ...
    self.filename = [NSString stringWithFormat:@"%@_%@.%@",type,timeStr,fileType];
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.assetWriter == nil) && !isVideo) {
            ...
            NSString *videoName = [self getUploadFile_type:@"video" fileType:@"mp4"];
            //self.filename = [videoName stringByDeletingLastPathComponent];
